...
import os
import time 
#LAST SAVED FRAMES: RMSE1: 0.36 RMSE2: 0.30
...
def calibrate_camera_for_intrinsic_parameters(images_prefix):
    ...
    for i, frame in enumerate(images):
        ...
        if ret == True:
            ...
            if len(sys.argv) >= 3 and sys.argv[2] == "UseSavedFrames":
                print("Skipping sample number:", i, "of detected keypoints...")
            else:
                cv.imshow('img', frame)
                k = cv.waitKey(0)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        print('ERROR: Call with settings filename: "python3 calib.py calibration_settings.yaml"')
        print("Optional uses: UseSavedFrames as arg3")
        quit()
   ...
   #get the world to camera0 rotation and translation
    R_W0, T_W0 = get_world_space_origin(cmtx0, dist0, os.path.join('frames_pair', 'camera0_9.png'))
    #get rotation and translation from world directly to camera1
    R_W1, T_W1 = get_cam1_to_world_transforms(cmtx0, dist0, R_W0, T_W0,
                                              cmtx1, dist1, R1, T1,
                                              os.path.join('frames_pair', 'camera0_9.png'),
                                              os.path.join('frames_pair', 'camera1_9.png'),)
